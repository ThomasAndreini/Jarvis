void getnparseforecast() {
  ...
  if (firstrun == 1) {
    firstrun = 0;//se e' il primo avvio azzera la variabile
    indx = 0;
    while (indx < 5) {
      delay(2000);
      ...
      if(indx==5)loop();//se indx e' arrivata a 5 tentativi non riusciti esci dal loop e dalla funzione
      indx++;
    }
  }
  if (timeout > 10) {//se dopo 11 tentativi non ha successo esce
  ...
  for (int j = 0; j < num_elements_fr; j++) {
    while (!Serial1.find(forecast[j])) {
      Serial.println(F("attesaforec")); // trova le parti interessate
    }
